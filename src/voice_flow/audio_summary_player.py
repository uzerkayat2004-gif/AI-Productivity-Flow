...
def player_url(token: str, *, base_url: str = "http://127.0.0.1:8991", depth: str = "balanced", title: str = "") -> str:
    params = {"id": token, "depth": depth}
    if title:
        params["title"] = title
    return f"{base_url.rstrip('/')}/audio-summary-player.html?{urllib.parse.urlencode(params)}"
...
def main() -> None:
    ...
    try:
        ...
        # Compact, sleek dimensions: 420 x 295 (min 360 x 250)
